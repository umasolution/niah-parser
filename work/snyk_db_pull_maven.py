...
class snyk_db():
    ...
    def fetch_packages_details(self, packagename, url, groupid, artifactid):
        page = requests.get(url)
        soup = BeautifulSoup(page.text, "html.parser")
        
        github = ''

        home_url = "https://mvnrepository.com/artifact/%s" % packagename.replace(":", "/")
        results = {}
    ...
    def indialize(self):
        ...
        i = 1
        while True:
            url = "https://security.snyk.io/vuln/maven/%s" % i
            page = requests.get(url)
            soup = BeautifulSoup(page.text, "html.parser")
            ...
            for tabls in soup.findAll('table', {'class': 'vue--table vulns-table__table'}):
                ...
                for tr in tabls.findAll('tr'):
                    if j == 0:
                        ...
                        a_tag_regex = re.findall(r'https:\/\/snyk\.io\/vuln\/maven%3A(.*)%3A(.*)', str(a_tag))
                        groupid = a_tag_regex[0][0]
                        artifactid = a_tag_regex[0][1]
                        packagename = "%s:%s" % (groupid, artifactid)
                        if packagename not in packages:
                            packages.append(packagename)
                            print(packagename)
                            res = self.fetch_packages_details(packagename, a_tag, groupid, artifactid)
                            results.append(res)
            i = i + 1
            if i == 120:
                break
        
        with open("maven_vulns.json", "w") as f:

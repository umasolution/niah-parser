...
class snyk_db():
    ...
    def fetch_packages_details(self, packagename, url, vendor, product):
        ...
        if vendor:
                home_url = "https://www.npmjs.com/package/%s/%s" % (vendor, product)
        else:
                home_url = "https://www.npmjs.com/package/%s" % (product)
    ...
    def indialize(self):
        ...
        i = 1
        while True:
            url = "https://security.snyk.io/vuln/npm/%s" % i
            page = requests.get(url)
            soup = BeautifulSoup(page.text, "html.parser")
                ...
                for tr in tabls.findAll('tr'):
                    if j == 0:
                        ...
                        try:
                                ...
                                if re.findall(r'https:\/\/snyk\.io\/vuln\/npm%3A%40(.*)%2F(.*)', str(a_tag)):
                                        a_tag_regex = re.findall(r'https:\/\/snyk\.io\/vuln\/npm%3A%40(.*)%2F(.*)', str(a_tag))
                                        ...
                                        a_tag_regex = re.findall(r'https:\/\/snyk\.io\/vuln\/npm%3A(.*)', str(a_tag))
                                        vendor = ''
                                        product = a_tag_regex[0]
                                        packagename = "%s" % (product)
                                        if packagename not in packages:
                                                packages.append(packagename)
                                                print(packagename)
                                                res = self.fetch_packages_details(packagename, a_tag, vendor, product)
        ...
        with open("npm_vulns.json", "w") as f:

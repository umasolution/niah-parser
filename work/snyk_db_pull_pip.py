...
class snyk_db():
    ...
    def fetch_packages_details(self, packagename, url, product):
        page = requests.get(url)
        soup = BeautifulSoup(page.text, "html.parser")
        
        github = ''

        home_url = "https://pypi.org/project/%s/" % (product)
    ...
    def indialize(self):
        ...
        i = 1
        while True:
            url = "https://security.snyk.io/vuln/pip/%s" % i
            page = requests.get(url)
            soup = BeautifulSoup(page.text, "html.parser")
                ...
                for tr in tabls.findAll('tr'):
                    if j == 0:
                        ...
                        try:
                                ...
                                if re.findall(r'https:\/\/snyk\.io\/vuln\/pip%3A(.*)', str(a_tag)):                              
                                        a_tag_regex = re.findall(r'https:\/\/snyk\.io\/vuln\/pip%3A(.*)', str(a_tag))
                                        product = a_tag_regex[0]
                                        packagename = "%s" % (product)
                                        if packagename not in packages:
                                                packages.append(packagename)
                                                print(packagename)
                                                res = self.fetch_packages_details(packagename, a_tag, product)
                                                results.append(res)
                        except:
                                pass
            i = i + 1
        ...
        with open("pip_vulns.json", "w") as f:
